...
import { checkLifecycleStatus, LifecycleAdvice, checkPhotoReminder } from '../logic/lifecycleEngine';
import { calculateMoonPhase, getMoonPhaseName, getMoonPhaseNameFromPhase, getMoonPhaseEmoji } from '../logic/lunarCalendar';
import { CheckCircle2, Circle, Calendar, Droplets, Shovel, Scissors, FlaskConical, Camera, Sparkles, Loader2, Sprout, X, PlusCircle, AlertCircle, Clock, Gauge, Scale, Star, ShoppingBasket, Snowflake, Sun, Box, Flower2, LayoutGrid, Users, History, Leaf, Shield, CheckCircle, XCircle, Moon, AlertTriangle } from 'lucide-react';
              ...
              <div key={task.id} className={`group relative flex flex-col p-5 rounded-2xl border transition-all duration-200 ${task.completed ? 'bg-gray-50 border-gray-100 opacity-70' : 'bg-white border-green-50 shadow-sm hover:shadow-md'}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(() => {
                      const photoReminder = checkPhotoReminder(task);
                      if (!photoReminder || !photoReminder.needsPhoto) return null;
                      return (
                        <div className={`mt-3 p-3 rounded-lg border-l-4 ${
                          photoReminder.isUrgent 
                            ? 'bg-red-50 border-red-400' 
                            : 'bg-blue-50 border-blue-400'
                        }`}>
                          <div className="flex items-center gap-2">
                            <AlertTriangle size={16} className={photoReminder.isUrgent ? 'text-red-600' : 'text-blue-600'} />
                            <p className={`text-sm font-medium ${photoReminder.isUrgent ? 'text-red-800' : 'text-blue-800'}`}>
                    ...
                    {task.images && task.images.length > 0 && (
                      <div className="mt-4">
                        <p className="text-xs font-bold text-gray-400 uppercase mb-2">Timeline Progressi</p>
                        <div className="space-y-3 mb-3">
                          {task.images.map((img, idx) => {
                            const photoDate = idx === 0 
                              ? new Date(task.date) 
                              : task.lastPhotoDate 
                                ? new Date(task.lastPhotoDate) 
                                : new Date(task.date);
                            const daysFromStart = Math.floor((photoDate.getTime() - new Date(task.date).getTime()) / (1000 * 60 * 60 * 24));
                              ...
                              <div key={idx} className="flex gap-3 items-start bg-gray-50 p-3 rounded-lg border border-gray-200">
                                ...
                                <div className="flex-1">
                                  <div className="flex items-center justify-between mb-1">
                                    <p className="text-xs font-bold text-gray-700">Foto #{idx + 1}</p>
                                    <p className="text-xs text-gray-500">{photoDate.toLocaleDateString('it-IT')}</p>
                                  </div>
                                  {daysFromStart > 0 && (
                                    <p className="text-xs text-gray-500">+{daysFromStart} giorni dalla semina/trapianto</p>
                                  )}
                                  {idx === 0 && (
                                    <p className="text-xs text-green-600 font-medium mt-1">Foto iniziale</p>
                                  )}
                                </div>
                              </div>
                            );
                    ...
                    <div className="flex gap-3 mt-3 pt-3 border-t border-dashed border-gray-100 flex-wrap">
                      <label className={`flex items-center gap-2 text-xs font-bold cursor-pointer px-3 py-1.5 rounded-lg transition-colors ${photoNeeded ? 'bg-blue-100 text-blue-700 hover:bg-blue-200' : 'bg-green-50 text-green-600 hover:text-green-700'}`}>
                        ...
                        {(() => {
                          const photoReminder = checkPhotoReminder(task);
                          if (photoNeeded && photoReminder) {
                            return (
                              <span>
                                AGGIORNA PROGRESSI
                                {photoReminder.daysSinceLastPhoto > 0 && (
                                  <span className="ml-1 text-xs">({photoReminder.daysSinceLastPhoto} gg fa)</span>
                                )}
                              </span>
                            );
                          }
