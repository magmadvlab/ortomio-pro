...
import { getCompanionWarnings, suggestCompanions } from '../logic/companionPlantingEngine';
...
const VisualGardenPlanner: React.FC<VisualGardenPlannerProps> = ({
  ...
  const getTaskStatus = (task: GardenTask): 'valid' | 'collision' | 'warning' => {
    ...
    // Check companion planting warnings
    if (task.gridPosition) {
      const master1 = masterDataMap.get(task.plantName);
      if (master1) {
        for (const otherTask of activeTasks) {
          if (otherTask.id === task.id || !otherTask.gridPosition) continue;
          const master2 = masterDataMap.get(otherTask.plantName);
          if (!master2) continue;
          
          // Check if they're close enough to matter (within 100cm)
          const dx = otherTask.gridPosition.x - task.gridPosition.x;
          const dy = otherTask.gridPosition.y - task.gridPosition.y;
          const distance = Math.sqrt(dx * dx + dy * dy);
          
          if (distance < 100) { // Within 1 meter
            const warning = getCompanionWarnings(task, otherTask, master1, master2);
            if (warning.hasWarning && warning.severity === 'Critical') {
              return 'collision'; // Treat harmful companions as collision
            }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
              ...
              return (
                <g key={task.id}>
                  ...
                  <circle
                    ...
                    r={footprint.radius}
                    fill={status === 'collision' ? '#fee2e2' : status === 'warning' ? '#fef3c7' : selectedTaskId === task.id ? '#bfdbfe' : '#dcfce7'}
                    stroke={color}
                    strokeWidth={status === 'collision' ? '3' : selectedTaskId === task.id ? '3' : '2'}
                    opacity={isDragging ? 0.7 : 1}
                    style={{ cursor: 'move' }}
                    onMouseDown={(e) => {
                      if (e.detail === 2) {
                        // Double click: select for companion suggestions
        ...
        {selectedTaskId && (() => {
          ...
          const master = masterDataMap.get(selectedTask.plantName);
          if (!master) return null;
          
          const allMasters = getAllMasterSheets();
          const suggestions = suggestCompanions(master, allMasters);
          ...
          return (
            <div className="border-t border-blue-200 bg-blue-50 p-4">
              <h3 className="font-bold text-sm text-blue-900 mb-2 flex items-center gap-2">
                <CheckCircle size={16} />
                Piante consigliate per {selectedTask.plantName}:
              </h3>
              {suggestions.length > 0 ? (
                <div className="space-y-2">
                  {suggestions.map((sug, idx) => (
                    <div key={idx} className="bg-white p-2 rounded-lg border border-blue-200">
                      <p className="font-bold text-sm text-blue-900">{sug.plant.commonName}</p>
                      <p className="text-xs text-gray-600">{sug.rule.reason}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-gray-600">Nessun suggerimento disponibile</p>
              )}
              <button
              ...
              <div className="text-sm">
                ...
                <ul className="text-red-700 space-y-1">
                  {activeTasks.map(task => {
                    ...
                    const collisions = checkAllCollisions(task, activeTasks, masterDataMap);
                    const collisionMessages = collisions.map((c, idx) => (
                      <li key={`${task.id}-collision-${idx}`}>
                        {c.collision.message}
                      </li>
                    ));
                    ...
                    const master1 = masterDataMap.get(task.plantName);
                    if (master1) {
                      const companionWarnings: JSX.Element[] = [];
                      for (const otherTask of activeTasks) {
                        if (otherTask.id === task.id || !otherTask.gridPosition) continue;
                        const master2 = masterDataMap.get(otherTask.plantName);
                        if (!master2) continue;
                        ...
                        if (distance < 100) {
                          const warning = getCompanionWarnings(task, otherTask, master1, master2);
                          if (warning.hasWarning) {
                            companionWarnings.push(
                              <li key={`${task.id}-companion-${otherTask.id}`} className={warning.severity === 'Critical' ? 'text-red-700' : 'text-orange-700'}>
                                {warning.message}
                              </li>
                            );
                          }
                      ...
                      return [...collisionMessages, ...companionWarnings];
                    }
