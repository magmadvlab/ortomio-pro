/**
 * PlannerAIChat Fixed - Versione semplificata senza loop di rendering
 * Risolve il problema di "rimane in rendering" dopo il click
 */
...
interface PlannerAIChatFixedProps {
  garden?: any
  tasks?: any[]
  isOpen: boolean
  onToggle: () => void
...
export default function PlannerAIChatFixed({ garden, tasks, isOpen, onToggle }: PlannerAIChatFixedProps) {
  const [messages, setMessages] = useState<ChatMessage[]>([])
  const [inputValue, setInputValue] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [responseCache] = useState<Map<string, any>>(new Map())
  const messagesEndRef = useRef<HTMLDivElement>(null)
  const inputRef = useRef<HTMLInputElement>(null)
  const [initialized, setInitialized] = useState(false)

  // Inizializza messaggi solo una volta quando si apre
  useEffect(() => {
  ...
  // Scroll to bottom quando cambiano i messaggi
  useEffect(() => {
  ...
  const generateAIResponse = useCallback((question: string, garden: any, tasks: any[]) => {
...
Puoi essere più specifico nella tua domanda per consigli più mirati!`,
  ...
  const handleSendMessage = useCallback(async (message?: string) => {
    ...
    try {
      ...
      if (responseCache.has(cacheKey)) {
        ...
        aiResponse = generateAIResponse(messageText, garden, tasks || [])
  ...
  }, [inputValue, isLoading, garden, tasks, generateAIResponse, responseCache])
