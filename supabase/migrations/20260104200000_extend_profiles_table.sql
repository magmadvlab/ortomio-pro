...
-- Add indexes for seed_inventory performance optimization (solo se le colonne esistono)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'seed_inventory' AND column_name = 'species_name') THEN
    CREATE INDEX IF NOT EXISTS idx_seed_inventory_garden_species ON seed_inventory(garden_id, species_name);
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'seed_inventory' AND column_name = 'expiry_year') 
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'seed_inventory' AND column_name = 'expiry_month') THEN
