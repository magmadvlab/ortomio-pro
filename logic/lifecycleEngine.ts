...
/**
 * Controlla se è necessario scattare una foto per tracciare i progressi
 * Suggerisce foto ogni 15 giorni
 */
export const checkPhotoReminder = (task: GardenTask): {
  needsPhoto: boolean;
  daysSinceLastPhoto: number;
  message: string;
  isUrgent: boolean;
} | null => {
  // Solo per piante attive (semina o trapianto)
  ...
  // Se non c'è mai stata una foto e sono passati almeno 7 giorni dalla semina/trapianto
  if (!task.lastPhotoDate && daysAlive >= 7) {
    return {
      needsPhoto: true,
      daysSinceLastPhoto: daysAlive,
      message: `È il momento di fotografare ${task.plantName} per tracciare i progressi!`,
      isUrgent: daysAlive >= 15
    };
  }
  
  // Se c'è una foto, controlla quando è stata scattata
  if (task.lastPhotoDate) {
    const lastPhoto = new Date(task.lastPhotoDate);
    const today = new Date();
    const daysSince = Math.floor((today.getTime() - lastPhoto.getTime()) / (1000 * 60 * 60 * 24));
    
    // Se sono passati 15 o più giorni, serve una nuova foto
    if (daysSince >= 15) {
      return {
        needsPhoto: true,
        daysSinceLastPhoto: daysSince,
        message: `Sono passati ${daysSince} giorni dall'ultima foto. Fotografa ${task.plantName} per vedere i progressi!`,
        isUrgent: daysSince >= 20
      };
    }
    
    // Se mancano 3 giorni o meno, avvisa che sta per scadere
    const daysUntilNext = 15 - daysSince;
    if (daysUntilNext <= 3 && daysUntilNext > 0) {
      return {
        needsPhoto: false,
        daysSinceLastPhoto: daysSince,
        message: `Prossima foto tra ${daysUntilNext} giorni`,
        isUrgent: false
      };
    }
  }
