-- =====================================================
-- DEPLOY MULTI-TENANCY SYSTEM - SAFE VERSION
-- =====================================================
-- Versione sicura che non dipende dalla tabella gardens
-- Può essere applicata anche se la tabella gardens non esiste
...
-- =====================================================
-- GARDEN ASSIGNMENTS (SAFE VERSION)
-- =====================================================

CREATE TABLE IF NOT EXISTS garden_assignments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  garden_id UUID NOT NULL, -- Rimosso il riferimento FK per ora
...
DROP POLICY IF EXISTS "Owners can manage invitations" ON organization_invitations;
...
DROP POLICY IF EXISTS "Members can view their assignments" ON garden_assignments;
...
DROP POLICY IF EXISTS "Owners can manage assignments" ON garden_assignments;
...
DROP POLICY IF EXISTS "Users can view their API keys" ON api_keys;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '🎉 MULTI-TENANCY SYSTEM DEPLOYED SUCCESSFULLY!';
    RAISE NOTICE '';
    RAISE NOTICE 'Created:';
    RAISE NOTICE '  • 6 tables (organizations, roles, members, invitations, assignments, api_keys)';
    RAISE NOTICE '  • 12+ RLS security policies';
    RAISE NOTICE '  • 12+ performance indexes';
    RAISE NOTICE '  • System roles creation function';
    RAISE NOTICE '  • Automatic triggers';
    RAISE NOTICE '';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '  1. Go to /app/settings in your application';
    RAISE NOTICE '  2. Create your first organization';
    RAISE NOTICE '  3. Invite team members';
    RAISE NOTICE '  4. Configure API keys';
    RAISE NOTICE '';
    RAISE NOTICE 'NOTE: If you have a gardens table, you can add the FK constraint later:';
    RAISE NOTICE 'ALTER TABLE garden_assignments ADD CONSTRAINT fk_garden_assignments_garden_id FOREIGN KEY (garden_id) REFERENCES gardens(id) ON DELETE CASCADE;';
    RAISE NOTICE '';
END $$;