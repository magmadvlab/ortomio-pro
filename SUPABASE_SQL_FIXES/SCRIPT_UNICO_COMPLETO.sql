-- =====================================================
-- SUPABASE DATABASE LINTER FIXES - SCRIPT UNICO COMPLETO
...
RETURNS TABLE (row_id UUID, row_name TEXT, crop_name TEXT, status TEXT)
...
RETURNS TABLE (total_trees BIGINT, healthy_trees BIGINT, diseased_trees BIGINT, avg_health NUMERIC)
LANGUAGE plpgsql SET search_path = public
...
RETURNS TABLE (plant_id UUID, canonical_name TEXT, synonym TEXT)
LANGUAGE plpgsql SET search_path = public
...
RETURNS TABLE (plant_id UUID, plant_code TEXT, position INTEGER, status TEXT)
LANGUAGE plpgsql SET search_path = public
...
-- 41-80: Altre funzioni (condensate per brevità)
CREATE OR REPLACE FUNCTION public.get_row_operation_stats(p_row_id UUID) RETURNS TABLE (operation_type TEXT, operation_count INTEGER, last_operation_date TIMESTAMP) LANGUAGE plpgsql SET search_path = public AS $$ BEGIN RETURN QUERY SELECT po.operation_type, COUNT(*), MAX(po.operation_date) FROM plant_operations po JOIN garden_plants gp ON gp.id = po.plant_id WHERE gp.row_id = p_row_id GROUP BY po.operation_type; END; $$;
...
CREATE OR REPLACE FUNCTION public.generate_plant_code(p_row_code TEXT, p_position INTEGER) RETURNS TEXT LANGUAGE plpgsql SET search_path = public AS $$ BEGIN RETURN p_row_code || '-' || LPAD(p_position::TEXT, 3, '0'); END; $$;
...
SELECT '✅ TUTTE LE CORREZIONI APPLICATE CON SUCCESSO!' as status;
