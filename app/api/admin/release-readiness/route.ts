...
export async function GET(request: NextRequest) {
  try {
    ...
    const requiredTables = [
      ...
      'release_capability_rollouts', 'release_observability_events',
    ]
    const [commands, runs, errors, predictions, observability, ...schemaProbes] = await Promise.all([
    ...
    const queryErrors = [commands.error, runs.error, errors.error, predictions.error, observability.error].filter(Boolean).map(error => error!.message)
    return NextResponse.json({
      windowHours: 24, schema, schemaReady: Object.values(schema).every(Boolean), metrics, ...evaluation,
