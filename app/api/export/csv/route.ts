...
export async function GET(request: NextRequest) {
  try {
    ...
    const content = renderExportCsv(envelope)
    const checksum = sha256Hex(content)
    const { error: auditError } = await supabase.from('export_audit_log').insert({
      user_id: user.id, garden_id: gardenId, dataset, format: 'csv', schema_version: EXPORT_SCHEMA_VERSION,
    ...
    return new NextResponse(content.buffer as ArrayBuffer, { headers: {
      ...
      'Content-Disposition': `attachment; filename="ortomio-${dataset}-${new Date().toISOString().slice(0, 10)}.csv"`,
