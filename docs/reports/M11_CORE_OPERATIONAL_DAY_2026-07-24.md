...
- test locali aggiornati: type-check verde e persistenza 56/56.
...
- le voci M11 pianificate scendono da 58 a zero; nessuna voce release resta non classificata.
...
- il servizio di tracking piante duplicato e solo in memoria, non importato dal runtime, e' stato rimosso; il writer canonico resta `plantOperationsService`.
...
Le parti locali `O25-O26` e tutto il debito sorgente assegnato a M11 sono completati. M11 resta parziale finche' `O27-O28`, cioe' la sequenza completa con ruoli reali e la riconciliazione, non vengono eseguiti su staging.
