...
| M15 | `[-]` parziale | O38-O39 e O41-O42 completati localmente; O40 chiuso con modello unico PRO senza piani/limiti | Prove staging O38-O42 e lifecycle commerciale `O43` |
...
- **Risultato parziale:** registrazione, schema organizzazioni/ruoli/inviti e UI censiti; eliminato il log del token invito. Il 26/07/2026 O38 e' stato completato localmente: la UI passa da `POST /api/organizations/provision`, l'owner deriva dalla sessione server e la RPC `provision_organization` crea organizzazione, ruoli e membership Owner in una sola transazione. O39 e' stato completato localmente subito dopo: creazione/lista inviti server-side senza esposizione token, delivery Resend tracciata, pagina di accettazione e RPC atomica vincolata all'email autenticata. O40 e' stato chiuso per decisione prodotto esplicita: esiste una sola versione PRO senza piani o limiti commerciali; i gate server e UI legacy sono stati resi inerti/rimossi. O41 implementa la procedura amministrativa single-PRO: profilo di fatturazione, emissione fattura, registrazione pagamento e rinnovo atomico con audit. O42 implementa sospensione e riattivazione atomiche con snapshot di membership/chiavi, enforcement RLS e audit. Restano configurazione provider e prove E2E staging prima dello stato `[x]`.
- **Evidenza:** commit `19cb061` e `M15_COMMERCIAL_LIFECYCLE_GAPS_2026-07-24.md`.
- **Residuo:** prove staging O38-O42, cancellazione, retention e accesso assistenza auditato.
...
| O40 | M15 | ~~Implementare licenze, piani e limiti~~ **Chiuso 26/07/2026 per decisione prodotto:** unica versione PRO, nessun piano/limite commerciale; gate legacy rimossi | Contratto single-PRO verificato, nessuna capability nascosta per tier |
| O41 | M15 | **Implementazione locale completata 26/07/2026:** procedura amministrativa single-PRO con profilo fatturazione, fattura, pagamento, rinnovo e audit; resta prova staging su due aziende | Ciclo economico documentato |
| O42 | M15 | **Implementazione locale completata 26/07/2026:** sospensione/riattivazione atomica, snapshot membership e chiavi, enforcement RLS e audit; resta prova staging | Accessi e dati coerenti |
| O43 | M15 | Implementare cancellazione, retention e accesso assistenza auditato | Lifecycle di uscita verificato |
| O45 | M05 | **Eliminare gli 8 file orfani verificati il 25/07/2026** (0 importer confermato via grep, sostituto vivo identificato per ciascuno — vedi nota M05 sopra): `components/planner/ProfessionalCalendar.tsx` (->`TaskCalendar.tsx`), `components/shared/EnhancedDashboard.tsx` (->`HomeDashboard.tsx`), `components/irrigation/IrrigationZonesWidget.tsx` e `components/irrigation/IrrigationDashboardWidget.tsx` (->`IrrigationZoneManager.tsx`/`ProfessionalIrrigationDashboard.tsx`), `components/OnboardingTier.tsx` (->step 1-7 reali), `components/vineyard/VineyardPruningManager.tsx` (feature migrata su `/app/mechanical-work?filter=Pruning`), `components/MigrationWizard.tsx` (tool pre-cloud obsoleto, non un doppione), `services/cultivationOrchestrator.ts` (973 righe — **non un doppione a pari merito**: e' un disegno monolitico "piano di coltivazione seme->raccolto" piu' vecchio e strutturalmente piu' povero della pipeline modulare oggi live, perche' calcola la data di raccolto da una tabella fissa di giorni per archetipo, ignorando lat/lng/suolo/altitudine/ostacoli in ogni firma di metodo; il sostituto reale e modulare e' `logic/seedlingTimelineEngine.ts::calculateSeedlingTimeline` (usa davvero `garden.coordinates.latitude`, sistemato in T01 lotto 8) + `services/transplantOrchestrationService.ts` + `services/taskExecutionOrchestratorService.ts`, tutti vivi e verificati). Ancora da verificare con lo stesso metodo prima di aggiungerli qui: `services/complianceAIService.ts`, `services/dominanceIntegrationService.ts`, `components/Dashboard.tsx` (emersi zero-importer nel lotto 12, non ancora analizzati per eventuali sostituti). Nuovo candidato emerso nel lotto 13: `components/compliance/RecallProcedure.tsx` (zero importer, non ancora analizzato). **Deliberatamente rimandato a fine sessione su richiesta esplicita dell'utente** ("questo aspetto lo lasciamo alla fine") — non eseguire senza conferma esplicita quando si riprende questa voce. | Tutti i file confermati rimossi, type-check/build/test verdi, nessun importer residuo |
