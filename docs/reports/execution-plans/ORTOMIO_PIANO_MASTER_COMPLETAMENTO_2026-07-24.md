...
| Scoperte O45-O60 | **14** | **0** | **2** | Sono aperti O48 (sicurezza credenziali provider) e O60 (fonti dati pianta/suolo e resa attesa in `prescriptionMapsService.ts`). Le altre 14 scoperte sono chiuse e pubblicate. |
...
strutturalmente irraggiungibili — registrato qui come osservazione, non
corretto, perche' decidere da dove questi segnali dovrebbero arrivare e' una
decisione agronomica/di prodotto, non di tipizzazione.
