...
| O46 | M09 | **Scoperto 26/07/2026 durante la prova Semenzaio:** Production rispondeva `42703` perché mancavano `seedling_batches.source`, `purchase_date` e `nursery_name`. Migrazione `20260726180000_seedling_batch_sources.sql` applicata transazionalmente e registrata in history il 26/07: query SQL `history_ok/source_ok/purchase_date_ok/nursery_name_ok = true`; probe PostgREST successivo `HTTP 200`. PR #68 pronta; restano merge e prova applicativa dei due percorsi. | Migrazione applicata e registrata in Production; creazione semina interna e piantina acquistata entrambe verificate |
