...
- the irrigation-water quality layer is now starting to become operational too:
  - `services/irrigationWaterQualityService.ts` now resolves an operational water profile per zone from irrigation-system source plus recent salinity, pH, and bicarbonate sensors, returning quality band, score, leaching fraction, risk flags, and recommendations without forcing a schema migration
  - `services/advancedIrrigationService.ts` now folds that profile into water-requirement calculation, so high-salinity water can increase operational irrigation need through a leaching fraction, while confidence and agronomic reasoning now account for water source and quality constraints
  - `types/irrigation.ts` now exposes reusable water-quality profile contracts, allowing irrigation decisions to carry source, salinity, pH, bicarbonates, and quality-band context instead of treating water only as a neutral volume input
- the same water-quality contract now starts influencing nutrition and fertigation too:
  - `services/advancedNutritionService.ts` now aggregates irrigation-water quality by garden, detects fertigation exposure, and surfaces a nutrition water-quality insight in dashboard and analytics payloads
  - `components/nutrition/ProfessionalNutritionDashboard.tsx` and `components/nutrition/NutritionAnalytics.tsx` now expose that insight so low-quality water is visible as a plausible cause of lower efficacy, unstable mixes, or timing friction instead of remaining hidden in irrigation only
  - this is still an early transversal step, but it matters: pH, salinity, and bicarbonates now begin to influence not only irrigation volume but also nutrition diagnostics and operator awareness
- the crop-kernel itself is now less generic on open-field families:
  - `data/agronomicCropProfiles.ts` now differentiates winter cereals, field brassicas, broadacre legumes, artichoke, and industrial/extensive crops instead of collapsing too many field crops into a few mixed profiles
  - aliases now route crops such as frumento, orzo, avena, brassicacee, lenticchia, favino, soia, mais, girasole, colza, and carciofo to more coherent agronomic packages
  - this matters because phenology, water strategy, nutrition timing, health priorities, and quality metrics are now less likely to be inherited from an unrelated vertical or an over-generic crop family
- the soil-water layer is starting to distinguish useful depth and salinity accumulation too:
  - `services/soilAnalysisService.ts` now derives effective rootable depth, drainage class, and estimated salinity accumulation risk in addition to field capacity, wilting point, infiltration, and compaction
  - `services/advancedIrrigationService.ts` now carries those fields into `soilWaterBalance`, so reasoning can distinguish nominal root depth from effective depth and flag slow-drainage or salinity-prone zones more explicitly
- the first ROI-aware decision layer now exists in the transversal queue:
  - `services/agronomicEconomicPriorityService.ts` now derives estimated intervention cost, cost of delay, protected value, net impact, and ROI ratio from irrigation, health, prescription, phenology, and director signals using transparent heuristics instead of leaving economics implicit
  - `services/agronomicPriorityService.ts` now accepts an economic summary, so agronomic ranking and confidence are no longer driven only by signals, profile resolution, and observed feedback but also by estimated economic asymmetry between acting and waiting
  - `services/agronomicActionQueueService.ts` now injects that ROI layer into every queue source and stores the resulting economic summary in queue metadata, making the transversal queue a real agronomic-economic prioritizer rather than a purely biological one
  - `services/agronomicQueueTaskService.ts`, `components/planner/AgronomicQueueTaskPanel.tsx`, and `components/director/DirectorBriefingWidget.tsx` now surface cost, delay, net impact, and ROI in planner/director UI so operators can see why an action is economically favored before turning it into a task
- the ROI layer now starts reading observed economics too instead of relying only on heuristics:
  - `services/agronomicMeasuredFeedbackService.ts` now stores execution-side economic signals in measured feedback, including irrigation estimated cost from actual liters/duration, nutrition total cost, harvest estimated unit price/value, and mechanical cost
  - `services/agronomicEconomicPriorityService.ts` now summarizes recent observed economics by focus/zone/plant and can override generic intervention-cost and protected-value assumptions with actual historical patterns when the system has them
  - `services/agronomicActionQueueService.ts` now feeds those observed economic summaries into queue prioritization, so repeated water, nutrition, and quality decisions can converge toward real site economics instead of staying on static coefficients
- the ROI layer now starts differentiating crop-package economics too:
  - `services/agronomicEconomicPriorityService.ts` now resolves the agronomic crop profile before scoring ROI, so delay cost, protected value, and intervention weight can change across winter cereals, brassicas, legumes, artichoke, industrial broadacre crops, orchard, olive grove, vineyard, and controlled-environment crops
  - this matters because the system no longer treats all quality- or water-related priorities as if they had the same economic elasticity; a vineyard veraison window, a brassica bulking issue, and a cereal grain-fill adjustment can now diverge economically even when agronomic priority is similar
  - `services/agronomicActionQueueService.ts` now passes profile IDs and crop hints into the economic scorer across irrigation, prescription, director, phenology, and health sources, making the queue both crop-aware and economics-aware instead of only focus-aware
