...
- **Design tokens — use these, do not invent new colors:** `ortomio-green-50/100/500/600/700/900`, `ortomio-earth-100/200/500/600/700`, `semantic-warning`, `semantic-info` (all defined in `index.css` and exposed via `tailwind.config.js`). Fonts: `font-display` (DM Sans) for headings, `font-body` (Inter, the Tailwind default already applies this via `body`) for copy, `font-mono` (JetBrains Mono) for data labels/badges — matches the "measured/estimated" motif from the copy spec.
