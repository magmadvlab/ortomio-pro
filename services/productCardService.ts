...
import { callAI, AIMessage } from './aiProxyService';
...
- Rispondi SOLO con JSON valido, senza testo aggiuntivo`;
  ...
  try {
    ...
    const messages: AIMessage[] = [
      {
        role: 'system',
        content: 'Sei un esperto agronomo specializzato in fertilizzanti e trattamenti fitosanitari biologici. Rispondi sempre in formato JSON valido.'
      },
      {
        role: 'user',
        content: prompt
    ...
    const aiResponse = await callAI(messages, {
      provider: (aiProvider as any) || 'groq',
      model: aiModel || 'llama-3.3-70b-versatile',
      temperature: 0.3,
      maxTokens: 2000
    });

    console.log('[ProductCardService] Risposta AI:', aiResponse.content.substring(0, 200));
