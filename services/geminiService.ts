...
export const getSpecificPlantDetails = async (query: string, lat: number, lng: number): Promise<SpecificPlantInfo | null> => {
  ...
  // DEBUG: Log solo in sviluppo per ridurre rumore
  if (process.env.NODE_ENV === 'development') {
    console.debug('[SEARCH] ===== STARTING SEARCH =====');
    console.debug('[SEARCH] Query:', query);
    console.debug('[SEARCH] Environment:', typeof window !== 'undefined' ? 'Client' : 'Server');
  }
  
  const varietyInfo = getVarietyInfoSync(query);
  if (process.env.NODE_ENV === 'development') {
    console.debug('[SEARCH] VarietyInfo result:', varietyInfo ? `✅ Found: ${varietyInfo.speciesId}` : '❌ Not found');
  }
  ...
  if (varietyInfo) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('[SEARCH] MasterGuide from variety:', masterGuide ? `Found: ${masterGuide.masterSheet.commonName}` : 'Not found');
    }
  } else {
    ...
    if (speciesFromVariety) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.debug('[SEARCH] MasterGuide from species:', masterGuide ? `Found: ${masterGuide.masterSheet.commonName}` : 'Not found');
      }
    } else {
      // Prova a cercare per nome specie
      masterGuide = generateCompleteGuideSync(query);
      if (process.env.NODE_ENV === 'development') {
        console.debug('[SEARCH] MasterGuide direct search:', masterGuide ? `Found: ${masterGuide.masterSheet.commonName}` : 'Not found');
      }
      if (masterGuide) {
        speciesName = query;
      }
  ...
  if (masterGuide && masterGuide.masterSheet) {
    if (process.env.NODE_ENV === 'development') {
      console.debug('[SEARCH] ✅ Returning LOCAL result for:', masterGuide.masterSheet.commonName);
    }
  ...
  if (process.env.NODE_ENV === 'development') {
    console.debug('[SEARCH] ⚠️ No local match found, falling back to AI');
  }
...
interface AnalyzeImageContext {
  ...
  lifecycleState?: 'Sowing' | 'Germination' | 'Nursing' | 'IntermediateRepotting' | 'Hardening' | 'Transplanting' | 'Production' | 'Disposal';
  daysActive?: number;
